	...
	for( uint i = 0u; i < SYS_COUNTOF( pServer->gameState.explosions ); ++i )
	{
		...
		if( pExplosion->time == GAMETIMESTEP )
		{
			...
			for( uint j = 0u; j < SYS_COUNTOF( pServer->gameState.bombs ); ++j )
			{
				...
				{
					ServerExplosion* pNewExplosion = find_free_explosion( pServer->gameState.explosions, SYS_COUNTOF( pServer->gameState.explosions ) );
					bomb_explode( pNewExplosion, pBomb );
